  ...
  <DashboardLayout pageTitle="Calendar">
    <div class="p-6">
      ...
      <div class="mb-6">
        ...
        <p class="text-gray-300">View all your items by expiration date</p>
      </div>
      ...
      <div class="bg-gray-900 rounded-2xl shadow-lg p-6 mb-6">
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="text-center">
              ...
              <p class="text-sm text-gray-400">{{ itemsThisMonth }} items this month</p>
            </div>
            ...
            >
              <ChevronRight :size="24" class="text-gray-300" />
            </button>
          </div>
          ...
          <div class="flex items-center gap-3">
            <button
              @click="goToToday"
              class="px-4 py-2 bg-gray-800 hover:bg-gray-800 text-gray-300 rounded-lg font-medium transition-colors"
            >
              Today
            </button>
            ...
            >
              <LayoutGrid v-if="viewMode === 'list'" :size="20" class="text-gray-300" />
              <List v-else :size="20" class="text-gray-300" />
            </button>
          </div>
        ...
        <div class="flex flex-wrap items-center gap-4 text-sm">
          <div class="flex items-center gap-2">
            ...
            <span class="text-gray-300">Expiring soon (within 7 days)</span>
          </div>
          <div class="flex items-center gap-2">
            <div class="w-3 h-3 rounded-full bg-orange-500"></div>
            <span class="text-gray-300">Expiring this month</span>
          </div>
          <div class="flex items-center gap-2">
            <div class="w-3 h-3 rounded-full bg-blue-500"></div>
            <span class="text-gray-300">Future</span>
          </div>
        </div>
      </div>
      ...
      <!-- Calendar View -->
      <div v-else-if="viewMode === 'month'" class="bg-gray-900 rounded-2xl shadow-lg overflow-hidden">
        
        <!-- Day Headers -->
        <div class="grid grid-cols-7 border-b border-gray-800 bg-gray-800/50/50">
          <div
            v-for="day in ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']"
            :key="day"
            class="p-4 text-center text-sm font-semibold text-gray-300"
          >
            {{ day }}
          </div>
        </div>
        ...
        <div class="grid grid-cols-7">
          <div
            ...
            :key="index"
            :class="[
              'min-h-32 border-b border-r border-gray-800 p-3 hover:bg-gray-800 transition-colors cursor-pointer',
              day.isToday ? 'bg-teal-50' : '',
              !day.isCurrentMonth ? 'bg-gray-800/50/50 opacity-50' : '',
          ...
          >
            ...
            <div class="flex items-center justify-between mb-2">
              <span
                :class="[
                  ...
                  day.isToday ? 'w-7 h-7 flex items-center justify-center bg-teal-500 text-white rounded-full' : 'text-gray-300',
            ...
            <div class="space-y-1">
              ...
              <div
                ...
                class="text-xs text-gray-400 font-medium px-2"
              >
      ...
      <div v-else class="space-y-4">
        <div
          ...
          :key="group.date"
          class="bg-gray-900 rounded-xl shadow-lg p-6"
        >
          <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <Calendar :size="20" class="text-teal-500" />
            {{ formatDateHeader(group.date) }}
            <span class="text-sm font-normal text-gray-400">({{ group.items.length }} items)</span>
          </h3>
          ...
          <div class="space-y-3">
            <div
              ...
              :key="item.id"
              class="flex items-center gap-4 p-4 bg-gray-800/50/50 rounded-lg hover:bg-gray-800 transition-colors cursor-pointer"
            ...
            >
              ...
              <div class="text-right">
                <p class="text-sm font-medium text-gray-300">{{ formatDate(item.expiration_date) }}</p>
        ...
        <!-- No items -->
        <div v-if="groupedItems.length === 0" class="bg-gray-900 rounded-xl shadow-lg p-12 text-center">
          <CalendarOff :size="48" class="mx-auto text-gray-400 mb-4" />
          <h3 class="text-xl font-bold text-white mb-2">No items this month</h3>
          <p class="text-gray-300 mb-6">Add items to see them on your calendar</p>
          <button
    ...
    >
      ...
      >
        <div class="bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6">
          <div class="flex items-center justify-between mb-6">
            <div>
              <h3 class="text-2xl font-bold text-white">{{ formatDateHeader(selectedDay.date) }}</h3>
              <p class="text-sm text-gray-300 mt-1">{{ selectedDay.items.length }} items expiring</p>
            </div>
            <button
              @click="selectedDay = null"
              class="p-2 hover:bg-gray-800 rounded-lg transition-colors"
            >
              <X :size="24" class="text-gray-300" />
            </button>
          </div>
          ...
          <div class="space-y-3">
            <div
              ...
              :key="item.id"
              class="flex items-center gap-4 p-4 bg-gray-800/50/50 rounded-xl hover:bg-gray-800 transition-colors cursor-pointer"
              @click="openItemDetails(item)"
            >
...
function getItemColor(item) {
  const days = getDaysUntil(item)
  if (days === null) return 'bg-gray-800 text-gray-300'
