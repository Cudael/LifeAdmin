<template>
  <DashboardLayout pageTitle="Account Settings">
    ...
    <div class="fixed top-0 left-1/4 w-[600px] h-[500px] bg-teal-500/10 blur-[150px] rounded-full pointer-events-none z-0 mix-blend-screen"></div>

    <div class="relative z-10 max-w-[1400px] mx-auto pb-12 animate-fade-in-up">

      <!-- PROFILE HEADER -->
      <div class="mb-8">
        <h1 class="text-3xl font-extrabold text-white mb-2 tracking-tight">Account Preferences</h1>
        <p class="text-slate-400 font-medium">Manage your secure identity, billing, and authentication settings.</p>
      </div>
          ...
          <div class="bg-slate-900/60 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white/5 p-3 sticky top-24">
            <nav class="flex flex-col gap-1">
              ...
              >
                ...
                <div 
                  class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-1/2 bg-teal-400 rounded-r-full transition-all duration-300 shadow-[0_0_10px_rgba(45,212,191,0.8)]"
                ...
                <component
                  ...
                  :class="active === item.key ? 'text-teal-400' : 'group-hover:text-slate-400'"
                />
        ...
        <section class="lg:col-span-9">
          <div class="bg-slate-900/60 backdrop-blur-xl rounded-[2rem] shadow-2xl border border-white/5 overflow-hidden min-h-[600px] flex flex-col">
            ...
            <div class="bg-white/[0.02] border-b border-white/5 px-8 py-6 flex items-center gap-4 relative overflow-hidden">
              <div class="absolute top-0 right-0 w-64 h-64 bg-teal-500/5 blur-[50px] rounded-full pointer-events-none"></div>
              
              <div class="w-12 h-12 rounded-2xl bg-teal-500/10 border border-teal-500/20 flex items-center justify-center shadow-inner relative z-10">
                <component :is="activeMenu.icon" :size="24" class="text-teal-400" />
              </div>
...
const menu = [
  {
    ...
    description: "Manage your secure identity",
    icon: User,
  ...
  },
  {
    key: "security",
    label: "Security & Auth",
    description: "Protect your vault access",
    icon: Shield,
